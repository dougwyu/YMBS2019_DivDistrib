...
install.packages(c("ade4","ape","beanplot","beepr","betapart","BiocManager","boral","car","conflicted","corrplot","data.table","iNEXT","iNextPD","metacoder","mvabund","phyloseq","RColorBrewer","renv","rmarkdown","SpeciesMix","sjmisc","tidyverse","UpSetR","vegan"), dependencies = TRUE)
...
communityAll <- t(communityAll_t) # t() transpose a table
TotalCounts <- c(colSums(communityAll)) 
TotalCounts # total number of reads per OTU added over all samples ("Cluster")
...
pCumSum # this is a ggplot2 object
pCumSum + xlim(0, 200)
ß
...
# after viewing the above figures, we chose 44 as the minimum OTU size
...
rowSums(commMBC.44)
commMBC44copies <- t(commMBC.44) # transpose to new table (OTUs in rows, samples in columns)
...
At this stage, we did some further filtering from 594 OTUs to 543 OTUs. We made a phylogenetic tree and observed a few OTUs on very long branches, and we removed those (see the paper for an explanation), and we also used the Naive Bayesian Classifier to classify reads to class, and we removed all OTUs that were not assigned to Arthropoda. This left us with 543 OTUs, and the new OTU table is stored inside the table 2014MBC_44reads_otu543_LandsatEnv.txt. This table *also* has all the environment metadata per sample. 

This is a safe way to store environment and community data together. Why is this safe?
...
# Metacoder heat trees. we used the taxonomic assignment (RDP classifier) results to build a taxon distribution of all OTUs. 
# Xiaoyang will explain how we went from 543 to 536 OTUs
...
# Comparing any number of treatments/groups. We are going to pairwise-compare all the habitats
...
# the first row is the number of sites per habitat
View(comm4inext)
